...
func _build_tiller_extension() -> void:
	# The shaft is a fixed 1.10 m Laser/ILCA-length tube. The sailor may slide
	...
	_add_tube_between("CarbonShaft", Vector3(0.0, 0.0, -0.030), Vector3(0.0, 0.0, -1.100), 0.011, _soft_black)
	...
	_add_tube_between("Grip", Vector3(0.0, 0.0, -0.560), Vector3(0.0, 0.0, -1.100), 0.014, _rubber)
	for ring_index in range(5):
		...
		rib.position.z = -0.600 - float(ring_index) * 0.120
...
func set_tiller_extension_grip_distance(distance: float) -> void:
	...
	set_meta(
		...
		clampf(
			...
			TILLER_EXTENSION_GRIP_MAX_DISTANCE
		)
	)
