...
const MIN_TOP_DOWN_DISTANCE := 4.5
const MAX_TOP_DOWN_DISTANCE := 32.5
const TOP_DOWN_ZOOM_STEP := 2.5
const TOP_DOWN_ORBIT_SENSITIVITY := 0.0035
const MIN_TOP_DOWN_ELEVATION := deg_to_rad(24.0)
const MAX_TOP_DOWN_ELEVATION := deg_to_rad(80.0)
...
var _top_down_orbit_yaw := 0.0
var _top_down_elevation := atan2(TOP_DOWN_DIRECTION.y, TOP_DOWN_DIRECTION.z)
var _top_down_orbit_dragging := false
...
func _ready() -> void:
	# Camera controls must still receive viewport events while non-interactive HUD
	# labels are visible over the game. The HUD itself uses MOUSE_FILTER_IGNORE.
	...
	var initial_top_down_basis := Basis(
		Vector3.UP,
		_top_down_follow_yaw + _top_down_orbit_yaw
	)
	top_down_camera.global_position = (
		target.global_position
		+ initial_top_down_basis * _top_down_direction() * _top_down_distance
	)
...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("toggle_camera"):
		...
		_apply_camera_mode()
	if _top_down_orbit_dragging and not Input.is_mouse_button_pressed(
		MOUSE_BUTTON_RIGHT
	):
		# A release consumed by UI or a mode switch must not leave orbit latched.
	...
	# Heading follow remains authoritative for the boat-relative frame. The user
	# orbit is a persistent relative offset layered on top, so releasing RMB keeps
	# the chosen view while translation and delayed heading tracking continue.
	...
	var desired_top_down := (
		target_position
		+ target_basis * _top_down_direction() * _top_down_distance
	)
	if teleported:
	...
	if not first_person_enabled:
		if event is InputEventMouseButton:
			if event.button_index == MOUSE_BUTTON_RIGHT:
				_top_down_orbit_dragging = event.pressed
				Input.mouse_mode = (
		...
		elif event is InputEventMouseMotion and _top_down_orbit_dragging:
			apply_top_down_orbit(event.relative)
			get_viewport().set_input_as_handled()
		return
...
func apply_top_down_orbit(relative: Vector2) -> void:
	# Keep this independent of pointer capture so RMB drag works with the visible
	# cursor and can be exercised deterministically by camera regression tests.
	# Horizontal input changes the boat-relative viewing side; vertical input
	# moves between a low chase view and a near-overhead view without reaching the
	# look_at() pole singularity.
	_top_down_orbit_yaw = wrapf(
		_top_down_orbit_yaw - relative.x * TOP_DOWN_ORBIT_SENSITIVITY,
		-PI,
		PI
	)
	_top_down_elevation = clampf(
		_top_down_elevation + relative.y * TOP_DOWN_ORBIT_SENSITIVITY,
		MIN_TOP_DOWN_ELEVATION,
		MAX_TOP_DOWN_ELEVATION
	)


func _top_down_direction() -> Vector3:
	var horizontal := cos(_top_down_elevation)
	return Vector3(
		0.0,
		sin(_top_down_elevation),
		horizontal
	)
